...
# Number of parallel threads - adjust based on your GPU VRAM
# For llama3:8b, you can typically run 4-8 instances on a 24GB GPU
...
if __name__ == "__main__":
    ...
    texts_to_process = [sr for sr in texts_to_classify if sr.identifier not in processed_ids]

    print(f"Total texts: {len(texts_to_classify)}")
    print(f"Already processed: {len(processed_ids)}")
    print(f"Remaining to process: {len(texts_to_process)}")
    print(f"Starting classification with {NUM_WORKERS} parallel workers using {MODEL_NAME}...")
    ...
    # Create a thread pool and process items in parallel
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        # Create a separate client for each thread
        futures = {
            executor.submit(
                process_single_item,
                search_result,
        ...
        for future in as_completed(futures):
            ...
            if result_data is not None:
                ...
                print(f"✓ Processed {completed_count}/{total_to_process} | "
