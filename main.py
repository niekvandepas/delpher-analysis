...
def main() -> None:
    ...
    regional_dishes_most_similar_word2vec = semantic_search(
        plain_texts, query_words=REGIONAL_DISHES_LIST, model="word2vec"
    )
    ...
    indo_authenticity_results = indo_authenticity(
        plain_texts, embedding_model=EmbeddingModel.FASTTEXT
    )
    ...
    indo_authenticity_results = indo_authenticity(
        plain_texts, embedding_model=EmbeddingModel.WORD2VEC
    )
    print(indo_authenticity_results)
    results_dict = asdict(indo_authenticity_results)
